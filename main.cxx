...
int main(int argc, char **argv)
{
    ...
    const auto outputType = ExistsArg(&argv[3], &argv[argc], "-u32") ? OutputDataType::UInt32 : ExistsArg(&argv[3], &argv[argc], "-byte") ? OutputDataType::Byte :
...
int WriteByteContent(const GenerateFileOptions &options, std::size_t numBytes, std::ifstream &inputFile, std::ofstream &outputFile);
...
int GenerateFile(const GenerateFileOptions &options)
{
    ...
    switch (options.OutputType)
    {
        ...
        case OutputDataType::UInt32:
            ...
            break;
        case OutputDataType::Byte:
            outputFile << "#include <cstddef>" << NewLine;
            break;
    }
    ...
    switch (options.OutputType)
    {
        ...
        case OutputDataType::Byte:
            return WriteByteContent(options, fileSizeInBytes, inputFile, outputFile);
    }
...
int WriteByteContent(const GenerateFileOptions &options, std::size_t numBytes, std::ifstream &inputFile, std::ofstream &outputFile)
{
    ...
    while (true)
    {
        ...
        outputFile << "std::byte(0x" << std::hex << std::setfill('0') << std::setw(2) << static_cast<uint32_t>(byte) << ")" << ((index % ValuesPerLine) == 0 ? "," : ", ");
    }
